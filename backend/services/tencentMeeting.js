...
class TencentMcpError extends Error {
...
function env(name, fallback) {
  return process.env[name] != null ? process.env[name] : (fallback == null ? '' : fallback);
}
...
function findFirst(obj, keys) {
  return findFirstSafe(obj, keys);
}

// Wrapper because findFirst uses a fast throw internally to avoid recursive code complexity.
...
class TencentMeetingClient {
  ...
  httpPost(body, options) {
    const self = this;
    const includeSession = !options || options.includeSession !== false;
    return new Promise(function (resolve, reject) {
      let url;
      try { url = new URL(self.url); } catch (e) { reject(new TencentMcpError('腾讯会议 MCP 地址无效', { code: 'BAD_URL' })); return; }
      ...
      const req = transport.request({
        ...
        res.on('end', function () {
          ...
          if (newSession) self.sessionId = Array.isArray(newSession) ? newSession[0] : newSession;
          let parsed;
          try {
            parsed = parseResponse(text, String(res.headers['content-type'] || ''), body.id);
          } catch (e) { reject(e); return; }
          if (res.statusCode < 200 || res.statusCode >= 300) {
            reject(new TencentMcpError('腾讯会议 MCP 请求失败', { code: res.statusCode, trace: extractTrace(parsed), raw: sanitizeForLog(text.slice(0, 2000)) }));
            return;
          }
          if (parsed && parsed.error) {
            reject(new TencentMcpError(parsed.error.message || '腾讯会议 MCP 返回错误', { code: parsed.error.code, data: parsed.error.data, trace: extractTrace(parsed) }));
            return;
          }
          resolve(parsed);
        });
      ...
      req.setTimeout(self.timeoutMs, function () {
        timedOut = true;
        req.destroy();
        reject(new TencentMcpError('腾讯会议 MCP 响应超时', { code: 'TIMEOUT' }));
      });
  ...
  async initialize() {
    ...
    for (let i = 0; i < versions.length; i += 1) {
      ...
      try {
        ...
        const response = await this.httpPost({ jsonrpc: '2.0', id: id, method: 'initialize', params: { protocolVersion: version, capabilities: {}, clientInfo: { name: 'tencent-meeting-booking', version: '1.0.3' } } }, { includeSession: false });
...
async function recoverCreatedMeeting(payload) {
...
async function updateMeeting(payload) {
  const timezone = payload.time_zone || 'Asia/Shanghai';
  const args = {
...
async function cancelMeeting(payload) {
...
function testConnection() { return client.healthCheck(); }

module.exports = { client, createMeeting, recoverCreatedMeeting, getMeeting, updateMeeting, cancelMeeting, testConnection, TencentMcpError };
